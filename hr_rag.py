...
def process_document_with_claude(file_content, file_name):
    ...
    Ensure your response is valid JSON that can be parsed by Python's json.loads() function.
    """
...
, '', json_str)
        ...
        try:
            ...
            if "metadata" not in extracted_data or "chunks" not in extracted_data:
                st.warning("JSON structure is incomplete. Falling back to simple chunking.")
                return fallback_chunking(file_content, file_name)
                
            return extracted_data
        except json.JSONDecodeError as e:
            st.warning(f"Could not parse Claude's JSON output: {str(e)}. Falling back to simple chunking.")
            return fallback_chunking(file_content, file_name)
